...
1. **Assurance gate bypass (2026-04-21).** `/paper-writing-codex` ran at
   `— effort: beast` silently skipped `/proof-checker-codex`,
   `/paper-claim-audit-codex`, and `/citation-audit-codex` because each phase's
   content detector could return negative and the outer prose said
   "audit is optional."
2. **Research wiki ingest no-op (2026-04-21).** `/research-wiki-codex init`
   created `research-wiki/papers/` but no paper ever landed there:
   `/arxiv-codex`, `/alphaxiv-codex`, `/deepxiv-codex`, `/semantic-scholar-codex`, `/exa-search-codex`,
   raw `Read`/`WebFetch` — none carried a wiki-ingest hook, and the two
   that did (`/research-lit-codex`, `/idea-creator-codex`) only had soft prose
...
| `forensics_gate.py` (in `/integrity-forensics-codex`, `/paper-writing-codex` Phase 5.9/6.0, `/resubmit-pipeline-codex`) | A (gate) | Typed policy gate + append-only obligations ledger for the Anti-Autoresearch launcher (deterministic slice in Codex-native sessions); when forensics is in play an unresolved helper blocks the Final Report / Overleaf push (never improvise the gate). `fresh` is the one-command downstream preflight; exit code is the source of truth |
...
| `research_wiki.py` (in `/research-wiki-codex` itself) | A (gate) | The SKILL is the wiki tool; missing helper means no functionality |
...
| `figure_renderer.py` | A (skill-local gate, single-skill) | `figure-spec-codex` cannot produce vector SVG output without the renderer. **Phase 3.1 move**: canonical location is `skills/figure-spec/scripts/figure_renderer.py`; `tools/figure_renderer.py` retained as `os.execv` shim for legacy resolver layers. |
| `experiment_queue/queue_manager.py`, `experiment_queue/build_manifest.py` | A (skill-local gate, single-skill) | `/experiment-queue-codex` cannot operate without these. **Phase 3.3 move**: canonical location is `skills/experiment-queue/scripts/{queue_manager.py, build_manifest.py}`; both `tools/experiment_queue/*.py` retained as `os.execv` shims for legacy resolver layers. |
...
invocations (one-off skills like `/arxiv-codex 2501.12345`).
...
- ✅ `/research-wiki-codex sync --arxiv-ids 2501.12345,1706.03762`
- ✅ `/research-wiki-codex sync --from-file ids.txt`
- ⚠️ `/research-wiki-codex sync` that scans `.aris/traces/` for arxiv IDs —
