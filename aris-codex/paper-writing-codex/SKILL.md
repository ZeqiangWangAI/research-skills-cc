...
In this hybrid pack, the pipeline itself is unchanged, but `paper-plan-codex` and `paper-write-codex` use Orchestra-adapted shared references for stronger story framing and prose guidance.
...
- **HUMAN_CHECKPOINT = false** — When `true`, the improvement loop (Phase 5) pauses after each round's review to let you see the score and provide custom modification instructions. When `false` (default), the loop runs fully autonomously. Passed through to `/auto-paper-improvement-loop-codex`.
- **ILLUSTRATION = `figurespec`** — Architecture/illustration generator for Phase 2b: `figurespec` (default, deterministic JSON→SVG via `/figure-spec-codex`, best for architecture/workflow/topology), `gemini` (AI-generated via `/paper-illustration-codex`, best for qualitative method illustrations; needs `GEMINI_API_KEY`), `mermaid` (Mermaid syntax via `/mermaid-diagram-codex`, free, best for flowcharts), or `false` (skip Phase 2b, manual only).

> Override inline: `/paper-writing-codex "NARRATIVE_REPORT.md" — venue: NeurIPS, illustration: gemini, human checkpoint: true`
> IEEE example: `/paper-writing-codex "NARRATIVE_REPORT.md" — venue: IEEE_JOURNAL`
...
Invoke `/paper-plan-codex` to create the structural outline:

```
/paper-plan-codex "$ARGUMENTS"
```
...
Invoke `/paper-figure-codex` to generate data-driven plots and tables:

```
/paper-figure-codex "PAPER_PLAN.md"
```
...
> **Scope:** `paper-figure-codex` covers data plots and comparison tables. Architecture diagrams, pipeline figures, and method illustrations are handled in Phase 2b below.
...
**When `illustration: figurespec`** (default) — invoke `/figure-spec-codex`:
```
/figure-spec-codex "[architecture/workflow description from PAPER_PLAN.md]"
```
...
**When `illustration: gemini`** — invoke `/paper-illustration-codex`:
```
/paper-illustration-codex "[method description from PAPER_PLAN.md or NARRATIVE_REPORT.md]"
```
...
**When `illustration: mermaid`** — invoke `/mermaid-diagram-codex`:
```
/mermaid-diagram-codex "[method description from PAPER_PLAN.md]"
```
...
> defeats. `/paper-write-codex` carries the full CONFIDENT PROSE, HONEST LIMITS
> contract.

### Phase 3: LaTeX Writing

Invoke `/paper-write-codex` to generate section-by-section LaTeX:

```
/paper-write-codex "PAPER_PLAN.md"
```
...
Invoke `/paper-compile-codex` to build the PDF:

```
/paper-compile-codex "paper/"
```
...
if results/*.json or results/*.csv or outputs/*.json exist:
    Run /paper-claim-audit-codex "paper/"
...
Invoke `/auto-paper-improvement-loop-codex` to polish the paper:

```
/auto-paper-improvement-loop-codex "paper/"
```
...
After `/auto-paper-improvement-loop-codex` finishes, **rerun** `/paper-claim-audit-codex` before the final report whenever the paper contains numeric claims and machine-readable raw result files exist.
...
(`/integrity-forensics-codex` launcher: SHA-pinned clone, evidence ledger, nine
...
📋 Submission audits required before Final Report:
   ...
   [ ] 1. /proof-checker-codex        → paper/PROOF_AUDIT.json
   [ ] 2. /paper-claim-audit-codex    → paper/PAPER_CLAIM_AUDIT.json
   [ ] 3. /citation-audit-codex       → paper/CITATION_AUDIT.json
...
> `ARIS_REPO="$(cd "$(readlink .agents/skills/paper-writing-codex)/../../.." && pwd)"`.
...
1. `/proof-checker-codex "paper/"` → writes `paper/PROOF_AUDIT.json` (emits
   `NOT_APPLICABLE` if the paper contains no theorems / lemmas / proofs)
2. `/paper-claim-audit-codex "paper/"` → writes `paper/PAPER_CLAIM_AUDIT.json`
...
3. `/citation-audit-codex "paper/"` → writes `paper/CITATION_AUDIT.json`
...
```
/idea-discovery-codex "direction"         ← Workflow 1: find ideas
implement                           ← write code
/run-experiment-codex                     ← deploy experiments
/auto-review-loop-codex "paper topic"     ← Workflow 2: iterate research
/paper-writing-codex "NARRATIVE_REPORT.md"  ← Workflow 3: you are here
                                         submit! 🎉

Or use /research-pipeline-codex for the Workflow 1+2 end-to-end flow,
then /paper-writing-codex for the final writing step.
```
