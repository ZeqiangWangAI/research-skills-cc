---
name: meta-apply-codex
description: "Privileged applier that LANDS meta-optimize / corpus-audit patches the user approved, with a fresh landing review and human approval. Base Codex review is same-family provisional. Use when the user says \"meta apply\", \"/meta-apply-codex\", \"land the staged patches\", \"应用优化\", after a /meta-optimize-codex run."
...
[`/meta-optimize-codex`](../meta-optimize-codex/SKILL.md) (and, later, `corpus-audit`) are
...
It is **human-invoked only.** It runs when the user explicitly types `/meta-apply-codex` after
...
1. **The human named THIS patch.** Apply only patches the user listed (`/meta-apply-codex 1,3`
...
[ -d "$PENDING" ] || { echo "Nothing staged. Run /meta-optimize-codex first."; exit 0; }
