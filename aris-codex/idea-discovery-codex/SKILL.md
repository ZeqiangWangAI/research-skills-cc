...
```
/research-lit-codex → /idea-creator-codex → /novelty-check-codex → /research-review-codex → /research-refine-pipeline-codex
...
- **ARXIV_DOWNLOAD = false** — When `true`, `/research-lit-codex` downloads the top relevant arXiv PDFs during Phase 1. When `false` (default), only fetches metadata. Passed through to `/research-lit-codex`.
...
- **RENDER_HTML = true** — When `true` (default), auto-render `idea-stage/IDEA_REPORT.md` to HTML at workflow end via `/render-html-codex`. Uses `--no-review` because the source already received novelty + same-family provisional review. Set `false` to skip.
...
> 💡 These are defaults. Override by telling the skill, e.g., `/idea-discovery-codex "topic" — ref paper: https://arxiv.org/abs/2406.04329` or `/idea-discovery-codex "topic" — compact: true`.
...
using the same resolver pattern as `/research-pipeline-codex`. If either helper is
...
--provisional-advances` (for example, `codex-gpt-6-astra`):
...
|---|---|
| `research-lit-codex` | `idea-stage/IDEA_REPORT.md#literature-landscape` |
| `idea-creator-codex` | `idea-stage/IDEA_REPORT.md#ranked-ideas` |
| `novelty-check-codex` | `idea-stage/IDEA_REPORT.md#novelty-verification` |
| `research-review-codex` | `idea-stage/IDEA_REPORT.md#external-critical-review` |
| `research-refine-pipeline-codex` | `refine-logs/FINAL_PROPOSAL.md` |

`novelty-check-codex` and `research-review-codex` are **reviewer-bearing phases**. A
...
positive verdict required by the run-state contract. For `novelty-check-codex`,
...
1. **If arXiv URL** — invoke `/arxiv-codex "ARXIV_ID" — download` to fetch the PDF, then read the first 5 pages.
...
Invoke `/research-lit-codex` to map the research landscape:

```
/research-lit-codex "$ARGUMENTS" — composed: idea-stage/IDEA_REPORT.md
```
...
- **User requests changes** (e.g., "focus more on X", "ignore Y", "too broad") → refine the search with updated queries, re-run `/research-lit-codex` with adjusted scope, and present again. Repeat until the user is satisfied.

### Phase 2: Idea Generation + Filtering + Pilots

Invoke `/idea-creator-codex` with the landscape context and `idea-stage/REF_PAPER_SUMMARY.md` if available:

```
/idea-creator-codex "$ARGUMENTS" — composed: idea-stage/IDEA_REPORT.md
```
...
```
/novelty-check-codex "[top idea 1 description]"
/novelty-check-codex "[top idea 2 description]"
```
...
```
/research-review-codex "[top idea with hypothesis + pilot results]" — composed: idea-stage/IDEA_REPORT.md
```
...
```
/research-refine-pipeline-codex "[top idea description + pilot results + reviewer feedback]"
```
...
- **User requests changes** → pass feedback to `/research-refine-codex` for another round.
...
- Reviewer score: X/10
- Next step: implement full experiment → /auto-review-loop-codex
...
## Next Steps
- [ ] /run-experiment-codex to deploy experiments from the plan
- [ ] /auto-review-loop-codex to iterate until submission-ready
- [ ] Or invoke /research-pipeline-codex for the complete end-to-end flow
```
...
- Next step: /experiment-bridge-codex or /research-refine-codex
```
...
no RECOMMENDED idea. `/experiment-bridge-codex` implements against this contract;
`/result-to-claim-codex` + `/ablation-planner-codex` read it as the claims source; session
...
After finalizing `idea-stage/IDEA_REPORT.md` (and the optional `IDEA_CANDIDATES.md`), invoke `/render-html-codex` on the report so the user has a single-file HTML view for tablet / phone reading:

```
/render-html-codex "idea-stage/IDEA_REPORT.md" --no-review
```
...
**Non-blocking**: if `/render-html-codex` fails (helper missing, secondary Codex agent unavailable, file write error), log the failure and continue. Skip entirely if `RENDER_HTML = false`.
...
```
/idea-discovery-codex "direction"         ← you are here (Workflow 1, includes method refinement + experiment planning)
/run-experiment-codex                     ← deploy experiments from the plan
/auto-review-loop-codex "top idea"        ← Workflow 2: iterate until submission-ready

Or use /research-pipeline-codex for the full end-to-end flow.
```
