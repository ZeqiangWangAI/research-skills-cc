...
|-------|--------|----------|
| `/arxiv-codex` | arXiv API | Latest preprints, cutting-edge unrefereed work |
| `/semantic-scholar-codex` | Semantic Scholar API | Published venue papers (IEEE, ACM, Springer) with citation counts |
| `/openalex-codex` | OpenAlex API | **Open citation graph, institutional affiliations, funding data, comprehensive metadata** |
| `/deepxiv-codex` | DeepXiv CLI | Layered reading: search, brief, section map, section reads |
| `/exa-search-codex` | Exa API | Broad web search: blogs, docs, news, companies, research papers |
| `/gemini-search-codex` | Gemini MCP / CLI | AI-powered broad literature discovery |
...
- **OPENALEX_FETCHER** — canonical name `openalex_fetch.py`, resolved per
  ...
  (Policy D1 — standalone `/openalex-codex` has no documented inline fallback,
  so unresolved helper terminates with an explicit error).

> Overrides (append to arguments):
> - `/openalex-codex "topic" — max: 20` — return up to 20 results
> - `/openalex-codex "topic" — year: 2023-` — papers from 2023 onward
> - `/openalex-codex "topic" — year: 2020-2023` — papers from 2020 to 2023
> - `/openalex-codex "topic" — type: article` — only journal articles
> - `/openalex-codex "topic" — type: preprint` — only preprints
> - `/openalex-codex "topic" — open-access` — only open access papers
> - `/openalex-codex "topic" — min-citations: 50` — minimum 50 citations
> - `/openalex-codex "topic" — sort: citations` — sort by citation count (descending)
> - `/openalex-codex "topic" — sort: date` — sort by publication date (newest first)
...
[ -f "$OPENALEX_FETCHER" ] || {
  echo "ERROR: openalex_fetch.py not resolved at \$ARIS_REPO/tools/, tools/, or ~/.codex/skills/openalex-codex/." >&2
  echo "       Fix: rerun install_aris_codex.sh, export ARIS_REPO, or copy the helper to ~/.codex/skills/openalex-codex/." >&2
...
```text
/semantic-scholar-codex "DOI:..."     — get S2 citation context and related papers
/arxiv-codex "arXiv:XXXX.XXXXX"      — fetch arXiv preprint if available
/research-lit-codex "topic" — sources: openalex, semantic-scholar  — combined multi-source review
/novelty-check-codex "idea"          — verify novelty against literature
```
...
- If OpenAlex API is unreachable or rate-limited, suggest using `/semantic-scholar-codex`, `/arxiv-codex`, or `/research-lit-codex "topic" — sources: web` as alternatives.
