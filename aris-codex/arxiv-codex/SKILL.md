...
- **ARXIV_FETCHER** — canonical name `arxiv_fetch.py`, resolved per
  ...
  (Codex-side chain: `$ARIS_REPO/tools/` → `tools/` → `~/.codex/skills/arxiv-codex/`).
  Policy D1 — if unresolved (canonical chain exhausted), fall back to inline Python.

> Overrides (append to arguments):
> - `/arxiv-codex "attention mechanism" - max: 20` - return up to 20 results
> - `/arxiv-codex "2301.07041" - download` - download a specific paper by ID
> - `/arxiv-codex "query" - dir: literature/` - save PDFs to a custom directory
> - `/arxiv-codex "query" - download: all` - download all result PDFs
...
```text
/research-lit-codex "topic"     - multi-source review: Zotero + Obsidian + local PDFs + web
/novelty-check-codex "idea"     - verify your idea is novel against these papers
```
...
- If the arXiv API is unreachable, report the error clearly and suggest using `/research-lit-codex` with `- sources: web` as a fallback
