---
name: proof-orchestrator-codex
description: "Manage a stateful, run-directory-based proof project with Codex: continuation across runs, run-local source bookkeeping, manual GPT Pro handoff packages when a local attempt stalls, and an optional DeepSeek second opinion as additional evidence only. Use when the user asks for proof-run orchestration, a GPT Pro handoff, or cross-run proof continuation — use /proof-writer-codex for ordinary proof drafting and /proof-checker-codex for rigorous verification or submission acceptance."
---
...
Existing paper workflows continue to use ARIS's canonical `/proof-checker-codex`;
...
local proof is difficult, and do not route ordinary `/proof-checker-codex` requests
here.
...
6. Read `references/audit-output-contract.md` and integrate the locally checked
   ...
   `/proof-checker-codex`'s canonical artifact).
...
- Keep existing `/proof-checker-codex` paper and assurance workflows unchanged. The optional DeepSeek branch is additional evidence, not their replacement.
