...
Standard score-based reviews (`/research-review-codex`, `/auto-paper-improvement-loop-codex`) tend to produce **balanced** weakness lists.  Each weakness gets ~equal attention, ranked CRITICAL > MAJOR > MINOR.  Empirically, this misses one specific failure mode: the **single most damaging argument** a reviewer would write in a rejection paragraph — the one sentence that, if a senior area chair reads it, kills the paper.
...
| `/research-review-codex` | "Deep technical review of methods + claims" | structured deep critique |
| `/proof-checker-codex` | "Is this theorem actually proved?" | per-step proof obligation audit |
| `/paper-claim-audit-codex` | "Does the paper report numbers truthfully?" | per-claim evidence verification |
| `/citation-audit-codex` | "Are citations real and used in correct context?" | per-entry KEEP/FIX/REPLACE/REMOVE |
| **`/kill-argument-codex`** | **"Write the single strongest rejection paragraph; then defend it."** | **attack memo + per-point defense + unresolved surfaced** |
...
- After 1-2 rounds of `/auto-paper-improvement-loop-codex` settled at a stable score, but before submission.  Surfaces what additional fixes would close the headline-attack gap.
...
This skill is most valuable for **theory papers** with ≥5 theorem-class environments (so the headline depends on real proof obligations).  For empirical papers without theorems, use `/research-review-codex` instead.
...
- **RENDER_HTML = true** — When `true` (default), auto-render `KILL_ARGUMENT.md` to HTML after writing the report via `/render-html-codex "<paper-dir>/KILL_ARGUMENT.md" --json "<paper-dir>/KILL_ARGUMENT.json"`. Uses **full review gate** (audit-class artifact). Set `false` to skip, or pass `— render html: false`. **Non-blocking**: failures don't invalidate the kill-argument verdict.
...
writing-level, queue for next /auto-paper-improvement-loop-codex round.
```
...
- `<paper-dir>/KILL_ARGUMENT.html` (when `RENDER_HTML = true`, default) — single-file HTML view auto-rendered via `/render-html-codex` with the JSON sidecar. Full review gate applies. **Non-blocking**: if `/render-html-codex` fails, the kill-argument verdict still counts as complete.
...
- **Detect-only by direct invocation; can be invoked by `/auto-paper-improvement-loop-codex` Step 5.5 which then merges unresolved findings into its fix list.**  When a user runs `/kill-argument-codex paper/` directly, the output is informational and the human decides whether to act.  When the skill is invoked from inside the auto-improvement loop, the loop reads `KILL_ARGUMENT.json`, deduplicates against its existing weakness list, and feeds novel `still_unresolved` points into Step 6 fixes — `/kill-argument-codex` itself never edits paper files.

## When NOT to Use

- Empirical papers without theorems / scope claims — `/research-review-codex` is more useful.  The skill emits `NOT_APPLICABLE` with `reason_code: not_theory_or_scope_paper` in this case.
...
- (`/auto-paper-improvement-loop-codex` Step 5.5 used to run this protocol inline; as of May 2026 it now invokes `/kill-argument-codex` and reads `KILL_ARGUMENT.json` instead, so there is no longer a "do not invoke from inside auto-loop" exclusion.)
...
This skill was extracted as a standalone primitive from `/auto-paper-improvement-loop-codex` Step 5.5 in May 2026, after the protocol proved valuable in surfacing headline-vs-body scope gaps that score-based reviews missed.  The attack-then-defense pattern was kept exactly because of empirical evidence that asking one model to "write the rejection memo" produces qualitatively different feedback than asking it to "review and grade" — the former forces commitment, the latter encourages hedging.
