...
node, a `/meta-optimize-codex` SKILL.md proposal — it must not store **operational
...
## Where ARIS uses it
- **`/research-wiki-codex`** (and `/idea-creator-codex` Phase-3 annotations): screen an
...
- **`/meta-optimize-codex`**: screen the rationale of a proposed SKILL.md change; never
