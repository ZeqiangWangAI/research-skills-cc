...
Every ARIS compute skill (`/run-experiment-codex`, `/experiment-queue-codex`,
`/serverless-modal-codex`, `/vast-gpu-codex`, `/qzcli-codex`) needs the same three things for a
...
| **Scheduler cluster** (Slurm/PBS; Qizhi-like platforms) | `/qzcli-codex` targets | `module load` or a container image built OFF-cluster and pulled (compute nodes often have **no internet** — pre-stage everything) | scheduler directives + container path in shared scratch (mind purge windows) |
| **Managed API** (serverless) | `/serverless-modal-codex`, Vast.ai templates | the provider's image definition (Modal `Image`, Vast template) — render the same spec into it | the provider's opaque image ref, recorded in the ledger |
