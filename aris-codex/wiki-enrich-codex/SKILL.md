...
`/wiki-enrich-codex` is the missing back half of `ingest_paper`: it reads each scaffolded paper page, fetches paper content from external sources via a graceful fallback chain (see Phase 2.3 for the full 5-source chain), and rewrites the 10 fillable TODO sections into 1-3 sentence prose summaries.
...
> 💡 Examples:
> - `/wiki-enrich-codex` — enrich every paper with ≥1 TODO section (most common usage)
> - `/wiki-enrich-codex vllm` — enrich a single paper by slug
> - `/wiki-enrich-codex all --force` — rewrite every paper from scratch (use when you've adopted a new style)
> - `/wiki-enrich-codex --source alphaxiv --max 5` — only use alphaxiv, only do 5 papers
> - `/wiki-enrich-codex missing --max 50` — bigger batch (watch token budget)
...
[ -d research-wiki/ ] || { echo "ERROR: research-wiki/ not found. Run /research-wiki-codex init first." >&2; exit 1; }

ARIS_REPO="${ARIS_REPO:-$(awk -F'\t' '$1=="repo_root"{print $2; exit}' .aris/installed-skills-codex.txt 2>/dev/null)}"
WIKI_SCRIPT=""
[ -n "$ARIS_REPO" ] && [ -f "$ARIS_REPO/tools/research_wiki.py" ] && WIKI_SCRIPT="$ARIS_REPO/tools/research_wiki.py"
[ -z "$WIKI_SCRIPT" ] && [ -f tools/research_wiki.py ] && WIKI_SCRIPT="tools/research_wiki.py"
[ -z "$WIKI_SCRIPT" ] && [ -f ~/.codex/skills/research-wiki-codex/research_wiki.py ] && WIKI_SCRIPT="$HOME/.codex/skills/research-wiki-codex/research_wiki.py"
...
| Reusable Ingredients | 1-3 bullets | Concrete | Techniques / datasets / insights from this paper that could be ported elsewhere. **Highest value for `/idea-creator-codex` — write carefully.** |
| Open Questions | 1-2 bullets | Question form | What the paper does NOT answer but raises |
| Claims | 1 line | Static | If no `claim:` edges in `graph/edges.jsonl` reference this paper, write the literal italic line: `_No claims tracked yet — populate via /proof-checker-codex._`. Else list claim node IDs. |
...
Re-ideation suggestion: <if ≥5 papers were enriched, recommend `/idea-creator-codex "topic"` so the freshly-filled `Reusable Ingredients` and `Limitations` feed brainstorming. `query_pack.md` is already rebuilt below — the user does NOT need to call `/research-wiki-codex query` manually.>
```

Also rebuild `query_pack.md` once at the end (single `python3 "$WIKI_SCRIPT" rebuild_query_pack research-wiki/` call) so `/idea-creator-codex` sees the new bodies on its next run.

## Output Protocols

> Follow the shared protocols:
> - **No `MANIFEST.md` entry.** This skill edits existing scaffolded pages in place rather than generating new artifacts. The audit trail lives in `research-wiki/log.md` (Step 2.6), with provenance per paper. Adding a `wiki-enrich-codex` stage to `shared-references/output-manifest.md` is out of scope for this PR.
...
- **Hard-fail on missing wiki / missing helper.** Do not silently create `research-wiki/` — if it's missing, the user is in the wrong cwd or hasn't run `/research-wiki-codex init`.
...
- **Don't auto-trigger `/idea-creator-codex`.** This skill builds the substrate; the user decides when to brainstorm next. Only *suggest* re-ideation in the final report.
...
```
/research-lit-codex "topic"               ← ingests papers as scaffolds (Step 6)
/wiki-enrich-codex                        ← THIS — fills paper bodies (you are here)
/research-wiki-codex lint                 ← health-check (orphans, contradictions, dead ideas)
/idea-creator-codex "direction"           ← reads query_pack, ideates on top of enriched wiki
/research-wiki-codex query "topic"        ← rebuild query_pack after big wiki changes
```

After a fresh `/research-pipeline-codex` run leaves Stage 1 Phase 1 done but Phase 2 not started (the failure mode that prompted this skill), the recovery path is:

```
/wiki-enrich-codex              # fill the paper TODOs ingest_paper left behind
/idea-creator-codex "..."        # now ideate with a wiki that actually has content
```
