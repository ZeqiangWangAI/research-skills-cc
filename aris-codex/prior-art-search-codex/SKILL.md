...
Adapted from `/research-lit-codex` for patent-specific searching.
...
2. **arXiv** (via `/arxiv-codex` if available, or WebSearch): Search for preprints
3. **Semantic Scholar** (via `/semantic-scholar-codex` if API key set, or WebSearch)
