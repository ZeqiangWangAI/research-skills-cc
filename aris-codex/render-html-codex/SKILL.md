...
# /render-html-codex: Markdown → single-file HTML for human reading

> **Markdown is for writers. HTML is for readers.** ARIS workflow nodes write Markdown (canonical, audit-trail-friendly, machine-parseable). `/render-html-codex` turns *selected* artifacts into a polished single-file HTML view for the human who actually has to read them. The Markdown stays the source of truth.

## When to use this skill

**Use `/render-html-codex` for** ARIS artifacts that have a real human reader:
...
[ -f ".agents/skills/render-html-codex/scripts/render_html.py" ] && RENDER_HTML=".agents/skills/render-html-codex/scripts/render_html.py"
...
[ -z "$RENDER_HTML" ] && [ -f ~/.codex/skills/render-html-codex/scripts/render_html.py ] && RENDER_HTML="$HOME/.codex/skills/render-html-codex/scripts/render_html.py"
[ -z "$RENDER_HTML" ] && {
  echo "ERROR: render_html.py not resolved at .agents/skills/render-html-codex/scripts/, skills/render-html/scripts/, \$ARIS_REPO/skills/render-html/scripts/, or ~/.codex/skills/render-html-codex/scripts/." >&2
  echo "       /render-html-codex cannot produce HTML output. Fix: rerun install_aris_codex.sh, export ARIS_REPO, or copy the render-html skill into ~/.codex/skills/." >&2
  exit 1
}
```
...
**If `should_review` is true**, fire a fresh `spawn_agent` call (NEVER `send_input`; pin `model: gpt-6-astra` + `reasoning_effort: xhigh` per `shared-references/reviewer-routing.md`) with the prompt below. The reviewer reads the source MD + generated HTML directly; it does **not** see this skill's intermediate state.

**Scope of review (narrow on purpose).** The HTML reviewer audits **render fidelity / safety / structure only** — not claim truthfulness. Claim audit belongs upstream (`/paper-claim-audit-codex`, `/research-review-codex`, `/result-to-claim-codex`). Specifically the reviewer checks:
...
**If `spawn_agent` is not available** (e.g., user runs `/render-html-codex` on a Codex-CLI-only setup where Codex MCP isn't wired): emit `verdict: REVIEW_UNAVAILABLE` to the sidecar, do not fabricate `PASS`, and tell the user the HTML was generated but **not** independently reviewed. The user can manually invoke `/research-review-codex` on the source MD or re-run with Codex MCP available.
...
**Phase 1 (this skill, currently shipped):** Opt-in only. Users explicitly call `/render-html-codex <artifact.md>` after a workflow completes. No existing skill changes. Lets us validate which HTML views are actually useful before automating.

**Phase 2 (later, planned):** Selectively auto-emit HTML at workflow termination:

- `/idea-discovery-codex` completes → also writes `idea-stage/IDEA_REPORT.html` (academic)
- `/auto-review-loop-codex` terminates → also writes `review-stage/AUTO_REVIEW.html` (academic, with `--state`)
- `/kill-argument-codex` already writes `.md + .json` → naturally extends to `+ .html`
- `/research-pipeline-codex` final report links to the HTML files above (doesn't re-render)
- `/research-wiki-codex render` subcommand generates `research-wiki/index.html` dashboard
- `/paper-writing-codex` **does not** auto-emit HTML (the final reader artifact is PDF)
...
4. Re-run `/render-html-codex <input> --template my_brand`.
...
- **The reviewer audits rendering, not research.** Claim truthfulness is owned upstream by `/paper-claim-audit-codex`, `/result-to-claim-codex`, `/research-review-codex`. The HTML reviewer asks: "did the renderer faithfully + safely convert this source?" — nothing more.
