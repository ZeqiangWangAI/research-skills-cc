...
Given a broad research direction from the user, systematically generate, validate, and rank concrete research ideas. Standalone, Phase 1's landscape survey is **inline** (WebSearch — it does not invoke `/research-lit-codex`); Phases 4-5 invoke `/novelty-check-codex`, `/run-experiment-codex`, and `/monitor-experiment-codex` for validation and pilots. For the full sub-skill pipeline (`/research-lit-codex` → idea generation → `/novelty-check-codex` → `/research-review-codex`), run `/idea-discovery-codex` (Workflow 1), which orchestrates this skill.
...
> 💡 Override via argument, e.g., `/idea-creator-codex "topic" — pilot budget: 4h per idea, 20h total`.
...
[ -z "$WIKI_SCRIPT" ] && [ -n "$ARIS_HOME" ] && [ -f "$ARIS_HOME/.codex/skills/research-wiki-codex/research_wiki.py" ] && WIKI_SCRIPT="$ARIS_HOME/.codex/skills/research-wiki-codex/research_wiki.py"
THREAT_SCANNER=""
...
2. **Novelty signal — ANNOTATE, do not eliminate**: do 2-3 targeted searches
   ...
   input for the Phase-4 reviewer, not a filter; full `/novelty-check-codex` runs in
...
1. **Novelty check**: Use the `/novelty-check-codex` workflow (multi-source search + GPT-6-Astra cross-verification) for each idea

2. **Critical review**: Use GPT-6-Astra via `send_input` (same agent):
   ```text
   send_input:
...
2. **Deploy in parallel**: Use `/run-experiment-codex` to launch pilots on different GPUs simultaneously:
   ```
...
3. **Collect results**: Use `/monitor-experiment-codex` to check progress. If any pilot exceeds PILOT_TIMEOUT_HOURS, kill it and collect partial results. Once all pilots complete (or timeout), compare:
...
- [ ] If confirmed, invoke /auto-review-loop-codex for full iteration
```
...
ideas without clobbering an existing idea whose `outcome` `/result-to-claim-codex` may already
have enriched. `--outcome` stays `pending` at creation (the experiment verdict is set
later by `/result-to-claim-codex`, never guessed here). If `WIKI_SCRIPT` is unavailable, the
...
```
/idea-creator-codex "direction"     → ranked ideas
/novelty-check-codex "top idea"     → deep novelty verification (already done in Phase 4, but user can re-run)
/research-review-codex "top idea"   → external critical feedback
implement                     → write code
/run-experiment-codex               → deploy to GPU
/auto-review-loop-codex             → iterate until submission-ready
```
