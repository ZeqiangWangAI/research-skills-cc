...
- **SOURCES = `all`** — Which literature sources to search. Options: `zotero`, `obsidian`, `local`, `web`, `semantic-scholar-codex`, `deepxiv`, `exa`, `gemini`, `openalex`, `all`. Full source table and selection rules: see `## Data Sources` below.
...
> 💡 Overrides:
> - `/research-lit-codex "topic" — paper library: ~/my_papers/` — custom local PDF path
> - `/research-lit-codex "topic" — sources: zotero, local` — only search Zotero + local PDFs
> - `/research-lit-codex "topic" — sources: web` — only search the web (skip all local)
> - `/research-lit-codex "topic" — sources: web, semantic-scholar` — also search Semantic Scholar for published venue papers
> - `/research-lit-codex "topic" — sources: all, deepxiv` — use default sources plus DeepXiv
> - `/research-lit-codex "topic" — arxiv download: true` — download top relevant arXiv PDFs
> - `/research-lit-codex "topic" — arxiv download: true, max download: 10` — download up to 10 PDFs
...
- **If `— sources:` is specified**: Only search the listed sources (comma-separated). Valid values: `zotero`, `obsidian`, `local`, `web`, `semantic-scholar-codex`, `deepxiv`, `exa`, `gemini`, `openalex`, `all`.
- **If not specified**: Default to `all` — search every available source in priority order (`semantic-scholar-codex`, `deepxiv`, `exa`, `gemini`, and `openalex` are excluded from `all`; they must be explicitly listed).

Examples:
```
/research-lit-codex "diffusion models"                        → all (default)
/research-lit-codex "diffusion models" — sources: all         → all
/research-lit-codex "diffusion models" — sources: zotero      → Zotero only
/research-lit-codex "diffusion models" — sources: zotero, web → Zotero + web
/research-lit-codex "diffusion models" — sources: local       → local PDFs only
/research-lit-codex "topic" — sources: obsidian, local, web   → skip Zotero
/research-lit-codex "topic" — sources: web, semantic-scholar  → web + Semantic Scholar API
/research-lit-codex "topic" — sources: deepxiv                → DeepXiv only
/research-lit-codex "topic" — sources: all, deepxiv           → default sources + DeepXiv
/research-lit-codex "topic" — sources: all, semantic-scholar  → default sources + Semantic Scholar API
/research-lit-codex "topic" — sources: exa                    → Exa only (broad web + content extraction)
/research-lit-codex "topic" — sources: all, exa               → default sources + Exa web search
/research-lit-codex "topic" — sources: gemini                 → Gemini only (AI-powered broad discovery)
/research-lit-codex "topic" — sources: all, gemini            → default sources + Gemini discovery
/research-lit-codex "topic" — sources: openalex               → OpenAlex only (open citation graph + institutions)
/research-lit-codex "topic" — sources: all, openalex          → default sources + OpenAlex citation graph
```
...
| 5 | **Semantic Scholar API** | `semantic-scholar-codex` | `$S2_FETCHER` resolves (canonical name `semantic_scholar_fetch.py`, per integration-contract §2 Codex chain) | Published venue papers (IEEE, ACM, Springer) with structured metadata: citation counts, venue info, TLDR. **Only runs when explicitly requested** |
...
4. **Export BibTeX**: Get citation data for relevant papers (useful for `/paper-write-codex` later)
...
[ -z "$ARXIV_FETCHER" ] && [ -f ~/.codex/skills/arxiv-codex/arxiv_fetch.py ] && ARXIV_FETCHER="$HOME/.codex/skills/arxiv-codex/arxiv_fetch.py"
...
**Semantic Scholar API search** (only when `semantic-scholar-codex` is in sources):
...
[ -z "$S2_FETCHER" ] && [ -f ~/.codex/skills/semantic-scholar-codex/semantic_scholar_fetch.py ] && S2_FETCHER="$HOME/.codex/skills/semantic-scholar-codex/semantic_scholar_fetch.py"
...
[ -z "$DEEPXIV_FETCHER" ] && [ -f ~/.codex/skills/deepxiv-codex/deepxiv_fetch.py ] && DEEPXIV_FETCHER="$HOME/.codex/skills/deepxiv-codex/deepxiv_fetch.py"
...
[ -z "$EXA_FETCHER" ] && [ -f ~/.codex/skills/exa-search-codex/exa_search.py ] && EXA_FETCHER="$HOME/.codex/skills/exa-search-codex/exa_search.py"
...
[ -z "$OPENALEX_FETCHER" ] && [ -f ~/.codex/skills/openalex-codex/openalex_fetch.py ] && OPENALEX_FETCHER="$HOME/.codex/skills/openalex-codex/openalex_fetch.py"
...
6. When the wiki helper is available, rebuild `query_pack.md` after updating literature entries so `/idea-creator-codex` can reuse the latest gaps and failed directions.
