...
This file is the source of truth for `/run-experiment-codex` and `/monitor-experiment-codex` to connect to vast.ai instances.

## Workflow

### Action: Provision (default)

Analyze the task, find the best GPU, and present cost-optimized options. This is the main entry point — called directly or automatically by `/run-experiment-codex` when `gpu: vast` is set.
...
To deploy: /run-experiment-codex (will auto-detect this instance)
To destroy when done: /vast-gpu-codex destroy <ID>
```
...
Set up the rented instance for a specific experiment. Called automatically by `/run-experiment-codex` when targeting a vast.ai instance.
...
```
/run-experiment-codex "train model"       ← detects gpu: vast, calls /vast-gpu-codex provision
  ↳ /vast-gpu-codex provision             ← analyzes task, presents options with cost
  ↳ user picks option               ← rent + setup + deploy
  ↳ /vast-gpu-codex destroy               ← auto-destroy when done (if auto_destroy: true)

/vast-gpu-codex provision                 ← manual: analyze task + show options
/vast-gpu-codex rent <offer_id>           ← manual: rent a specific offer
/vast-gpu-codex list                      ← show active instances
/vast-gpu-codex destroy <instance_id>     ← tear down, stop billing
/vast-gpu-codex destroy-all               ← tear down everything
```
