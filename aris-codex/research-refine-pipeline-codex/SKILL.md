---
name: research-refine-pipeline-codex
description: "Run an end-to-end workflow that chains `research-refine-codex` and `experiment-plan-codex`. Use when the user wants a one-shot pipeline from vague research direction to focused final proposal plus detailed experiment roadmap, or asks to \"\u4e32\u8d77\u6765\", build a pipeline, do it end-to-end, or generate both the method and experiment plan together."
---
...
1. `research-refine-codex` for method refinement
2. `experiment-plan-codex` for claim-driven validation planning

For stage-specific detail, read these sibling skills only when needed:

- `../research-refine-codex/SKILL.md`
- `../experiment-plan-codex/SKILL.md`
...
- If the proposal is missing, stale, or materially different from the current request, run the full `research-refine-codex` stage.
- If the proposal is already strong and aligned, reuse it and jump to experiment planning.
- If in doubt, prefer re-running `research-refine-codex` rather than planning experiments for the wrong method.

### Phase 1: Method Refinement Stage

Run the `research-refine-codex` workflow and keep its V3 philosophy intact:
...
Run the `experiment-plan-codex` workflow grounded in:
...
## Next Action
- Proceed to `/run-experiment-codex`
```
...
```
/research-refine-pipeline-codex -> one-shot method + experiment planning
/research-refine-codex   -> method refinement only
/experiment-plan-codex   -> experiment planning only
/run-experiment-codex    -> execution
```
