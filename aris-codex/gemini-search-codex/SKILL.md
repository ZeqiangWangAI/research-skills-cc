...
| `/gemini-search-codex` | Gemini MCP / CLI | **AI-powered broad literature discovery** — searches across multiple angles, aliases, and sub-problems |
...
> - `/gemini-search-codex "topic" — max: 20` — request up to 20 papers
> - `/gemini-search-codex "topic" — year: 2020-` — papers from 2020 onward
> - `/gemini-search-codex "topic" — code-only` — only papers with open-source code
> - `/gemini-search-codex "topic" — venues: NeurIPS,ICML,ICLR` — focus on specific venues
> - `/gemini-search-codex "topic" — model: gemini-3-flash-preview` — Gemini 3 Flash (faster, higher quota, less capable than Pro)
> - `/gemini-search-codex "topic" — model: auto-gemini-3` — auto-routes within the Gemini 3 family by load
> - `/gemini-search-codex "topic" — model: gemini-2.5-pro` — legacy (only if your `gemini-cli` < v0.40)
...
- **arXiv ID**: if available (for cross-reference with `/arxiv-codex`)
...
```text
/semantic-scholar-codex "topic"    — search published venue papers with citation counts
/arxiv-codex "arXiv:XXXX.XXXXX"   — fetch specific preprint details
/research-lit-codex "topic" — sources: gemini, semantic-scholar  — combined multi-source review
...
- **Gemini is a discovery source, not a database.** Its results may include papers it "knows about" from training data. Always cross-verify critical details (exact titles, venues, years) via `/semantic-scholar-codex` or `/arxiv-codex` when precision matters.
...
- If both MCP and CLI are unreachable, suggest using `/semantic-scholar-codex`, `/arxiv-codex`, or `/research-lit-codex "topic" — sources: web` as alternatives.
