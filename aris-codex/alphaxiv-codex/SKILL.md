...
**Do NOT use this skill for** topic discovery, broad literature search, or multi-paper surveys — use `/research-lit-codex` or `/arxiv-codex` instead.
...
> - `/alphaxiv-codex 2401.12345` — quick overview
> - `/alphaxiv-codex "https://arxiv.org/abs/2401.12345"` — auto-extract ID
> - `/alphaxiv-codex 2401.12345 - depth: src` — force LaTeX source inspection
> - `/alphaxiv-codex 2401.12345 - depth: abs` — force full markdown
...
```text
/arxiv-codex "PAPER_ID" - download          - download the PDF to local library
/deepxiv-codex "PAPER_ID" - section: Methods  - read a specific section progressively
/research-lit-codex "related topic"        - multi-source literature survey
/novelty-check-codex "idea from paper"     - verify novelty against this paper's area
```
## Key Rules
...
- **No PDF parsing**: This skill reads structured markdown and LaTeX source, not raw PDFs. For PDF content, suggest `/arxiv-codex` with download.
- **Rate limiting**: arXiv source download may rate-limit. If HTTP 429 occurs, wait 5 seconds and retry once. If still blocked, report the error and suggest `/deepxiv-codex` as alternative.
- **Complementary, not competing**: This skill complements `/arxiv-codex` (search + download) and `/deepxiv-codex` (progressive reading). Do not re-implement their functionality.

## Integration with Other Skills

### As enrichment in `/research-lit-codex`

`/research-lit-codex` can use this skill's Tier 1 (overview) as a fast enrichment step between search and deep analysis. After finding arXiv papers in Step 1, fetch AlphaXiv overviews to quickly assess relevance before committing to full-text reads:
...
After `/research-lit-codex`, `/novelty-check-codex`, or `/idea-discovery-codex` surface a specific paper, users can invoke `/alphaxiv-codex PAPER_ID` for a fast deep-dive without re-running the full survey.
