...
A long ARIS workflow (`/research-pipeline-codex`, `/paper-writing-codex`, `/idea-discovery-codex`)
...
| `skipped` | the phase does not apply to this run (e.g. `paper-writing-codex` when `AUTO_WRITE=false`) | executor (`set`) | terminal — a deterministic config decision, not a quality verdict |
