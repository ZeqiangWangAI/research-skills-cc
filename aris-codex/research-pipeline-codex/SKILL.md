...
- **ARXIV_DOWNLOAD = false** — When `true`, `/research-lit-codex` downloads the top relevant arXiv PDFs during literature survey. When `false` (default), only fetches metadata via arXiv API. Passed through to `/idea-discovery-codex` → `/research-lit-codex`.
- **HUMAN_CHECKPOINT = false** — When `true`, the auto-review loops (Stage 3) pause after each round's review to let you see the score and provide custom modification instructions before fixes are implemented. When `false` (default), loops run fully autonomously. Passed through to `/auto-review-loop-codex`.
- **REVIEWER_DIFFICULTY = medium** — How adversarial the reviewer is. `medium` (default): standard MCP review. `hard`: adds **Reviewer Memory** + **Debate Protocol**. `nightmare`: GPT reads repo directly via `codex exec` + memory + debate. Passed through to `/auto-review-loop-codex`.
- **CODE_REVIEW = true** — GPT-6-Astra xhigh reviews experiment code before deployment. Catches logic bugs before wasting GPU hours. Set `false` to skip. Passed through to `/experiment-bridge-codex`.
- **BASE_REPO = false** — GitHub repo URL to use as base codebase. When set, `/experiment-bridge-codex` clones the repo first and implements experiments on top of it. When `false` (default), writes code from scratch or reuses existing project files. Passed through to `/experiment-bridge-codex`.
- **COMPACT = false** — When `true`, generates compact summary files for short-context models and session recovery. Passed through to `/idea-discovery-codex` and `/experiment-bridge-codex`.
- **AUTO_WRITE = false** — When `true`, automatically invoke Workflow 3 (`/paper-writing-codex`) after Stage 4. `VENUE` is needed only when Stage 5 begins — a missing venue defers paper writing; it never blocks Stages 1-4. When `false` (default), Stage 4 generates `NARRATIVE_REPORT.md` and stops — user invokes `/paper-writing-codex` manually.
...
- **RENDER_HTML = true** — When `true` (default), auto-render `NARRATIVE_REPORT.md` to HTML at Stage 4 completion via `/render-html-codex`. Uses `--no-review` because Stage 3 already produced a traced same-family provisional review. Set `false` to skip. Rendering failure is non-blocking.
...
> 💡 Override via argument, e.g., `/research-pipeline-codex "topic" — AUTO_PROCEED: false, human checkpoint: true, difficulty: nightmare, code review: false, base repo: https://github.com/org/project, auto_write: true, venue: NeurIPS`.
...
```
/idea-discovery-codex → /experiment-bridge-codex → /auto-review-loop-codex → /paper-writing-codex (optional)
...
- New run: `python3 "$RUN_STATE" start . "$RUN_ID" --executor codex-gpt-6-astra --provisional-advances --phases "idea-discovery,experiment-bridge,auto-review-loop,summary,paper-writing"` (the `--provisional-advances` policy is what lets a same-family provisional verdict close a phase for resume — without it, mainline semantics apply and provisional phases stay open).
- Resume: `python3 "$RUN_STATE" resume . "$RUN_ID"`; restart the returned phase.
- Each phase: mark `running`, then `done --artifact <path>`.
- A fresh Codex reviewer PASS uses `mark-provisional --reviewer gpt-6-astra
...
- If `AUTO_WRITE=false`, mark `paper-writing-codex` as `skipped` after summary.
...
```
/idea-discovery-codex "$ARGUMENTS" — AUTO_PROCEED: $AUTO_PROCEED
```

This internally runs: `/research-lit-codex` → `/idea-creator-codex` → `/novelty-check-codex` → `/research-review-codex`
...
The user may:
- **Approve the idea** → proceed to Stage 2. `/experiment-bridge-codex` reads `refine-logs/EXPERIMENT_PLAN.md` already generated by `/idea-discovery-codex`.
- **Request changes** (e.g., "combine Idea 1 and 3", "focus more on X") → update the idea prompt with user feedback, re-run `/idea-discovery-codex` with refined constraints, and present again.
...
Once the idea is selected (automatically or by the user), delegate implementation and deployment to `/experiment-bridge-codex`:

```
/experiment-bridge-codex "$CHOSEN_IDEA_TITLE" — code review: $CODE_REVIEW, base repo: $BASE_REPO, compact: $COMPACT
```

> 💡 **Queue routing is automatic**: `/experiment-bridge-codex` Phase 4 routes each milestone by job count — ≤5 jobs → `/run-experiment-codex`, ≥10 jobs or teacher→student phase dependencies → `/experiment-queue-codex` (with OOM retry, wave gating, crash-safe state). No manual override is needed.
...
3. **Fresh-agent code review** — GPT-6-Astra xhigh reviews the implementation in a new context; base result is same-family provisional
...
5. Deploys full experiments — auto-routes by job count (≤5 → `/run-experiment-codex`, ≥10 → `/experiment-queue-codex` with OOM retry, wave gating, crash-safe state)
6. Collects initial results — parses outputs, updates `refine-logs/EXPERIMENT_TRACKER.md`, runs `/training-check-codex` if W&B is configured
7. Auto-plans ablations via `/ablation-planner-codex` if main results are positive
...
```
/monitor-experiment-codex [server]
```

Wait for `/experiment-bridge-codex` to complete and report its handoff summary before proceeding.
...
```
/auto-review-loop-codex "$ARGUMENTS — [chosen idea title], difficulty: $REVIEWER_DIFFICULTY"
```
...
- Venue: [VENUE or "not set — run /paper-writing-codex manually"]
...
**Skip this stage if `AUTO_WRITE=false` (default).** Present the `/paper-writing-codex` command for manual use:

```
📝 Research complete. To write the paper:
/paper-writing-codex "NARRATIVE_REPORT.md" — venue: <VENUE>, AUTO_PROCEED: $AUTO_PROCEED
```
...
- Next step: /paper-writing-codex "NARRATIVE_REPORT.md" — venue: [VENUE], AUTO_PROCEED: $AUTO_PROCEED
...
`/paper-plan-codex → /paper-figure-codex → /paper-write-codex → /paper-compile-codex → /auto-paper-improvement-loop-codex`
...
After Stage 4 finalizes `NARRATIVE_REPORT.md` (before paper writing branches), invoke `/render-html-codex` on the narrative report:

```
/render-html-codex "NARRATIVE_REPORT.md" --no-review
```
...
**Non-blocking**: if `/render-html-codex` fails (helper missing, file write error, etc.), log the failure and continue Stage 4 — the HTML view is a convenience artifact, not a pipeline prerequisite.
