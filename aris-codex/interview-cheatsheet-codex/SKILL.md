---
name: interview-cheatsheet-codex
...
# /interview-cheatsheet-codex — long-form Chinese ML/LLM interview prep
...
- **`--byline`** (default `"<Your Name>, <Affiliation>"`) — passed to `/render-html-codex --author`.
...
Invoke `spawn_agent` with `model: gpt-6-astra`, `reasoning_effort: xhigh`, and a fresh thread. Do not reuse prior reviewer context.
...
### Step 5 — Render via /render-html-codex

Call directly (do not invoke `/render-html-codex` as a sub-skill; call its python script — gives clear control):
...
```
✅ /interview-cheatsheet-codex "<TOPIC>" complete.
  ...
  Suggested commit message:
    docs(tutorials): add <Topic> cheat sheet (rendered via /render-html-codex)
...
|---|---|
| Executor/reviewer family | Codex drafts; fresh gpt-6-astra reviews (math/code stage and render stage), same-family provisional |
...
- Don't want HTML output — call `/render-html-codex` separately or skip Step 5

## Reference invocations

```
/interview-cheatsheet-codex "RLHF / DPO / PPO"
/interview-cheatsheet-codex "MoE (Mixture-of-Experts)" — effort: max
/interview-cheatsheet-codex "KV Cache + Speculative Decoding"
/interview-cheatsheet-codex "Long-context: RoPE / YaRN / NTK / MLA"
/interview-cheatsheet-codex "Distributed Training (DDP / FSDP / ZeRO / TP / PP)"
/interview-cheatsheet-codex "Quantization (GPTQ / AWQ / INT4 / FP8 / SmoothQuant)"
```
