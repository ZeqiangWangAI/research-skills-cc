...
This contract is referenced by `paper-writing-codex`, `paper-claim-audit-codex`, `citation-audit-codex`, `proof-checker-codex`, and the external verifier (canonical name `verify_paper_audits.sh`; callers resolve the actual path via `integration-contract.md` §2).
...
- `paper-writing-codex` can produce a final report without the submission verifier.
...
- `paper-writing-codex` runs `verify_paper_audits.sh` (resolved per `integration-contract.md` §2).
...
At `assurance: submission`, `paper-writing-codex` must treat verifier exit 1 as blocking.
