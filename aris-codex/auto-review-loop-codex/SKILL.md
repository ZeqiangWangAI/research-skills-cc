...
- **RENDER_HTML = true** — When `true` (default), auto-render `review-stage/AUTO_REVIEW.md` to HTML on loop termination via `/render-html-codex`. Uses `--no-review` because the loop already performed a traced same-family provisional review. Set `false` to skip.

> 💡 Override: `/auto-review-loop-codex "topic" — compact: true, human checkpoint: true, difficulty: hard`
...
- **Training quality check** — if W&B is configured, invoke `/training-check-codex` to verify training was healthy (no NaN, no divergence, no plateau). If W&B is not available, skip silently.
...
4. **Write method/pipeline description** to `review-stage/AUTO_REVIEW.md` under a `## Method Description` section — a concise 1-2 paragraph summary of the final method, architecture, and data flow. This serves as direct input for `/paper-illustration-codex`.
5. **Generate claims from results** — invoke `/result-to-claim-codex` to convert experiment results from `review-stage/AUTO_REVIEW.md` into structured paper claims. Output: `CLAIMS_FROM_RESULTS.md`. If `/result-to-claim-codex` is not installed, skip this step (no `CLAIMS_FROM_RESULTS.md` is produced; `/paper-plan-codex` extracts claims from the narrative as before) — but NEVER fabricate the file or its verdict. If it ran but its output starts with `verdict: REVIEW_UNAVAILABLE`, keep that file AS-IS (do not overwrite or paraphrase it) and record in `AUTO_REVIEW.md` that claims are UNADJUDICATED — downstream paper stages must not treat them as validated.
...
8. **Render HTML view** (if `RENDER_HTML = true`, default): invoke `/render-html-codex` on the cumulative review log:
   ```
   /render-html-codex "review-stage/AUTO_REVIEW.md" --no-review --state review-stage/REVIEW_STATE.json
   ```
   Pass `--state` explicitly when `REVIEW_STATE.json` exists (the helper does not auto-discover the sidecar). HTML lands at `review-stage/AUTO_REVIEW.html` with embedded source SHA256. **Non-blocking**: if `/render-html-codex` fails, log the error and continue — the HTML is a convenience, not a termination prerequisite.
