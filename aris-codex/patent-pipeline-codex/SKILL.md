...
```
/prior-art-search-codex → /patent-novelty-check-codex → /invention-structuring-codex → /claims-drafting-codex → /specification-writing-codex → /patent-review-codex → /jurisdiction-format-codex
     (search)           (verify)              (structure)             (claims)            (description)          (examiner)         (compile)
                                                                                              ├── /figure-description-codex
                                                                                              └── /embodiment-description-codex
```

**This is a parallel branch, not part of the linear research pipeline.** After `/idea-discovery-codex` produces validated ideas, the user can either:
- Go to `/experiment-bridge-codex` → `/auto-review-loop-codex` → `/paper-writing-codex` (publish track)
- Go to `/grant-proposal-codex` (funding track)
- Go to `/patent-pipeline-codex` (patent track) **<-- this skill**

```
                    ┌→ /experiment-bridge-codex → /auto-review-loop-codex → /paper-writing-codex  (publish track)
/idea-discovery-codex ────┤
                    ├→ /grant-proposal-codex → [get funded] → ...  (funding track)
                    └→ /patent-pipeline-codex → [file patent]       (patent track)
```
...
- **JURISDICTION = `CN`** — Target patent jurisdiction. Options: `CN` (CNIPA), `US` (USPTO), `EP` (EPO), `ALL` (generate all three). Override via argument (e.g., `/patent-pipeline-codex "invention — US"`).
...
> Override defaults via arguments: `/patent-pipeline-codex "invention — US, utility model"` or `/patent-pipeline-codex "invention — ALL, language: Chinese"`.
...
2. Read `IDEA_REPORT.md` if it exists (from `/idea-discovery-codex` -- can extract invention from research results)
...
Invoke `/prior-art-search-codex`:

```
/prior-art-search-codex "patent/INVENTION_BRIEF.md"
```
...
Invoke `/patent-novelty-check-codex`:

```
/patent-novelty-check-codex "patent/INVENTION_BRIEF.md"
```
...
Invoke `/invention-structuring-codex`:

```
/invention-structuring-codex "patent/INVENTION_BRIEF.md"
```
...
Invoke `/claims-drafting-codex`:

```
/claims-drafting-codex "patent/INVENTION_DISCLOSURE.md"
```
...
Invoke `/specification-writing-codex`:

```
/specification-writing-codex "patent/CLAIMS.md"
```

This writes the full specification section by section. Internally invokes `/figure-description-codex` (if user-provided figures exist) and `/embodiment-description-codex` for the detailed description. The specification-writing skill handles figure processing and embodiment writing as sub-skills.
...
Invoke `/patent-review-codex`:

```
/patent-review-codex "patent/"
```
...
Invoke `/jurisdiction-format-codex`:

```
/jurisdiction-format-codex "patent/"
```
...
```
User describes invention directly ──→ /patent-pipeline-codex

/idea-discovery-codex produces IDEA_REPORT.md ──→ /patent-pipeline-codex (extract patentable aspects)

/research-refine-codex produces FINAL_PROPOSAL.md ──→ /patent-pipeline-codex (from refined research idea)

/auto-review-loop-codex produces strong results ──→ /patent-pipeline-codex (patent the method)
```
