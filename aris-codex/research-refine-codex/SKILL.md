...
User input (PROBLEM + vague APPROACH)
  ...
  -> Optional handoff: /experiment-plan-codex for a detailed execution-ready experiment roadmap
```

## Constants

- **REVIEWER_MODEL = `gpt-6-astra`** — Reviewer model used via a secondary Codex agent.
...
> Override via argument if needed, e.g. `/research-refine-codex "problem | approach" -- max rounds: 3, threshold: 9`.
...
- Default to **1-3 core experiment blocks** and leave the full execution roadmap to `/experiment-plan-codex`.
...
Send the full proposal to GPT-6-Astra for an **elegance-first, frontier-aware, method-first** review. The reviewer should spend most of the critique budget on the method itself, not on expanding the experiment menu.
...
Send the revised proposal back to GPT-6-Astra in the **same agent**:
...
## Next Steps
- If READY: proceed to `/experiment-plan-codex` for a full experiment roadmap, then `/run-experiment-codex`
- If REVISE: manually address the remaining mechanism weaknesses, then re-run `/research-refine-codex`
- If RETHINK: revisit the core mechanism, possibly with `/idea-creator-codex`
```
...
Final proposal: refine-logs/FINAL_PROPOSAL.md
Suggested next step: /experiment-plan-codex
```
...
```
/research-refine-pipeline-codex              -> one-shot refine + experiment planning
/idea-creator-codex "direction"       -> candidate ideas
/research-refine-codex "PROBLEM: ... | APPROACH: ..."  <- you are here
/experiment-plan-codex                -> detailed experiment roadmap
/run-experiment-codex                 -> execute the chosen method
/auto-review-loop-codex               -> iterate on results and paper
```

Typical flow:

1. `/idea-creator-codex` or local reading gives you a problem and a vague method direction
2. `/research-refine-codex` turns that into an anchored, elegant, frontier-aware method plan
3. `/experiment-plan-codex` turns the final proposal into a detailed claim-driven experiment roadmap
4. `/research-refine-pipeline-codex` is the one-shot wrapper when the user wants both stages in a single request
5. `/run-experiment-codex` executes the chosen runs
