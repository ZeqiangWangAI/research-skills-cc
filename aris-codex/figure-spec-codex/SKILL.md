...
**Do NOT use for:**
- Data plots (bar/line/scatter) — use `/paper-figure-codex`
- Natural/qualitative illustrations — use `/paper-illustration-codex`
- Quick state-machine / flowchart — use `/mermaid-diagram-codex` (lighter syntax)
...
copied the canonical into `~/.codex/skills/figure-spec-codex/figure_renderer.py`
must now place it at `~/.codex/skills/figure-spec-codex/scripts/figure_renderer.py`
...
[ -z "$FIGURE_RENDERER" ] && [ -f ~/.codex/skills/figure-spec-codex/scripts/figure_renderer.py ] && FIGURE_RENDERER="$HOME/.codex/skills/figure-spec-codex/scripts/figure_renderer.py"
[ -z "$FIGURE_RENDERER" ] && [ -f ~/.codex/skills/figure-spec-codex/figure_renderer.py ] && FIGURE_RENDERER="$HOME/.codex/skills/figure-spec-codex/figure_renderer.py"  # pre-Phase-3.1 layout

[ -n "$FIGURE_RENDERER" ] || {
  echo "ERROR: figure_renderer.py not found at any of: \$ARIS_REPO/skills/figure-spec/scripts/, \$ARIS_REPO/tools/, tools/, ~/.codex/skills/figure-spec-codex/scripts/, ~/.codex/skills/figure-spec-codex/. Set ARIS_REPO, rerun install_aris_codex.sh, or copy the canonical \$ARIS_REPO/skills/figure-spec/scripts/figure_renderer.py next to this skill." >&2
  exit 1
}
...
- **`/paper-writing-codex`** (Workflow 3): when `illustration: figurespec` (default for architecture figures), this skill handles Phase 2b
- **`/paper-figure-codex`**: handles data plots; they complement each other (data + architecture = complete figure set)
- **`/paper-illustration-codex`**: fallback for figures that need natural/qualitative style (method illustrations with photos, qualitative result grids)
- **`/mermaid-diagram-codex`**: lighter alternative for simple flowcharts
