...
- **REVIEWER_MODEL = `gpt-6-astra`** — Model used via a secondary Codex agent for outline review. Must be an OpenAI model.
- **TARGET_VENUE = `ICLR`** — Default venue. User can override (e.g., `/paper-plan-codex "topic" — venue: NeurIPS`). Supported: `ICLR`, `NeurIPS`, `ICML`, `CVPR`, `ACL`, `AAAI`, `ACM`, `IEEE_JOURNAL` (IEEE Transactions / Letters), `IEEE_CONF` (IEEE conferences).
...
5. **CLAIMS_FROM_RESULTS.md** — structured claim judgment from `/result-to-claim-codex` (preferred if available)
...
## Next Steps
- [ ] /paper-figure-codex to generate all figures
- [ ] /paper-write-codex to draft LaTeX
- [ ] /paper-compile-codex to build PDF
```
