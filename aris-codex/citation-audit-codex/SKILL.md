...
This skill is the fourth layer of \aris{}'s evidence-and-claim assurance, complementing `experiment-audit-codex` (code), `result-to-claim-codex` (science verdict), and `paper-claim-audit-codex` (numerical claims). Together they form a bottom-up integrity stack from raw evaluation code to manuscript bibliography.

## When to Use This Skill

**Run before submission.** The right gating point is:
- After `paper-write-codex` has produced the LaTeX draft and bib file
- After `paper-claim-audit-codex` has verified numerical claims
- Before final `paper-compile-codex` for submission
...
- Verifier gates and downstream skills (`paper-writing-codex` Phase 6, `verify_paper_audits.sh`) MUST NOT treat the presence of `uncited_entries` as a blocking signal.
...
- **Always emit, never block** — this skill always writes `CITATION_AUDIT.json` with a verdict; the decision to block finalization lives in `paper-writing-codex` Phase 6 + `verify_paper_audits.sh`, driven by the `assurance` level. See "Submission Artifact Emission" below.
...
|-------|---------------|-----------------|
| `/experiment-audit-codex` | Evaluation code | Fake ground truth, self-normalized scores, phantom results |
| `/result-to-claim-codex` | Result-to-claim mapping | Claims unsupported by evidence |
| `/paper-claim-audit-codex` | Numerical claims in manuscript | Number inflation, best-seed cherry-pick, config mismatch |
| `/citation-audit-codex` | Bibliographic entries | Hallucinated refs, wrong-context citations, metadata errors |
...
- `CITATION_AUDIT.html` (when `RENDER_HTML = true`, default) — auto-rendered single-file HTML view via `/render-html-codex "CITATION_AUDIT.md" --json "CITATION_AUDIT.json"`. Full review gate. Sidecar `.review.json` carries render-fidelity verdict. **Non-blocking**: failures are logged and the audit MD + JSON remain the canonical outputs.
...
**Opt-in**: pass `— soft-only` (also accepts `— soft_only`) on invocation. This mode is designed for callers — notably `/resubmit-pipeline-codex` Phase 1 — that operate under a **hard "freeze the bib" constraint**: if a citation is wrong-context, soften the surrounding sentence; do **not** change, add, or remove the cite itself.
...
- If a downstream caller — including `paper-writing-codex` Phase 6 or any wrapper — proposes a bib edit while `--soft-only` is set, **refuse it**: emit a one-line refusal in the trace and continue to the next finding.
...
`paper-writing-codex` Phase 6 and `verify_paper_audits.sh` both rely on
...
- `suggestion` is advisory only; the verifier and `paper-writing-codex` Phase 6 do not block on it.
...
This skill never blocks by itself; `paper-writing-codex` Phase 6 plus the
verifier decide whether the verdict blocks finalization based on the
`assurance` level.

## See Also

- `/paper-claim-audit-codex` — sibling skill for numerical claim verification
- `/experiment-audit-codex` — sibling skill for evaluation code integrity
- `/result-to-claim-codex` — claim verdict assignment from results
