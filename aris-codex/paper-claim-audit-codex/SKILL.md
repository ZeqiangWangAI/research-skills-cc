...
|-------|-------------------|
| `/experiment-audit-codex` | Is the experiment code honest? (fake GT, normalization fraud) |
| `/result-to-claim-codex` | Does the data scientifically support this claim? |
| **`/paper-claim-audit-codex`** | **Does the paper report the data truthfully and precisely?** |
...
1. **After `/paper-write-codex`** — first check before improvement loop
2. **After `/auto-paper-improvement-loop-codex`** — recheck if improvement loop changed numbers
3. **Before submission** — final verification

## Integration with Other Skills

### Read by `/auto-paper-improvement-loop-codex` (if exists)
...
Same pattern as `/experiment-audit-codex`:
...
After writing `paper/PAPER_CLAIM_AUDIT.md` and `paper/PAPER_CLAIM_AUDIT.json`, invoke `/render-html-codex` on the audit report:

```
/render-html-codex "paper/PAPER_CLAIM_AUDIT.md" --json "paper/PAPER_CLAIM_AUDIT.json"
```
...
**Non-blocking**: if `/render-html-codex` fails (helper missing, secondary Codex agent unavailable, file write error), log the failure and treat the audit as complete — the JSON + MD verdict files are canonical; the HTML view is a human-reader convenience.
...
raw-results run emits `BLOCKED`. Silent skip is forbidden — `paper-writing-codex`
...
is for humans. The parent skill (`paper-writing-codex` Phase 6) plus the verifier
