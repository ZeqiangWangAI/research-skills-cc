...
- It **does not** generate outlines, speaker scripts, or new Beamer/PPTX from
  paper source. That is `/paper-slides-codex`'s job.
- It **does not** change figures or equations content.

If you do not yet have a deck, run `/paper-slides-codex` first. If you want to
change content, go back to `/paper-slides-codex` Phases 1-2 (or rewrite the outline
manually) — do not run `/slides-polish-codex` for that.
...
> 💡 Override examples
>
> - `/slides-polish-codex talk_pptx/talk.pptx — reference: talk_beamer/main.pdf — style: why-rf`
> - `/slides-polish-codex talk_beamer/ — reference: ./reference_talk.pdf — style: generic — effort: max`
> - `/slides-polish-codex talk.pptx — reference: ./why_rf_2025.pdf — interactive`
...
Note: existing skills like `/paper-slides-codex` may use a co-located state file
(e.g., `slides/SLIDES_STATE.json`). `/slides-polish-codex` keeps its state
...
Inherit color schemes from `/paper-slides-codex`. Polish-loop and font-scaling rules
unchanged.
...
- Runs **after** `/paper-slides-codex` (or any externally generated deck).
- Compatible with `/paper-poster-html-codex` workflow (same color discipline) but
  different output cadence.
- Uses the same `spawn_agent` MCP infrastructure as
  `/auto-paper-improvement-loop-codex`, `/research-review-codex`, etc.
- Does **not** call or compose with `/paper-slides-codex` content phases — strict
  separation.

## When NOT to Use

- Slides are still in **content drafting** phase — go back to `/paper-slides-codex`
  Phases 1-2.
- Complete redesign needed (different colors / template / aspect ratio) —
  re-run `/paper-slides-codex`, not polish.
...
When invoked via `/research-pipeline-codex` or another orchestrator, parameters
flow as:
