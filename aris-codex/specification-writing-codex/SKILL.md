---
name: specification-writing-codex
...
Adapted from `/paper-write-codex` for patent specifications. The specification supports the claims -- it is not a paper.
...
- `OUTPUT_FORMAT = "markdown"` — Markdown drafts; converted to filing format by `/jurisdiction-format-codex`
...
Invoke `/figure-description-codex` as a sub-skill if user has provided figures:
```
/figure-description-codex "patent/figures/"
```
...
Invoke `/embodiment-description-codex` as a sub-skill:
```
/embodiment-description-codex "patent/CLAIMS.md"
```
