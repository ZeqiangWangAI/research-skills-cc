...
2. **Original deck is preserved.** `/paper-slides-codex` produces a baseline deck; `/slides-polish-codex` writes a `_polished` versioned copy. The original Beamer + PPTX are never overwritten.
...
6. **Fresh-context independence.** Per-page Codex calls in `/slides-polish-codex` use fresh `spawn_agent` calls (no `send_input`) and are same-family provisional in the base mirror.
...
- **TALK_TYPE = `spotlight`** — Default talk format. Inherited by `/paper-slides-codex`.
- **TALK_MINUTES = 15** — Default duration. Inherited by `/paper-slides-codex`.
- **VENUE = `NeurIPS`** — Default venue (used by `/paper-slides-codex` color schemes when `— style` is not passed).
- **ASPECT_RATIO = `16:9`** — Inherited by `/paper-slides-codex`.
- **STYLE_PRESET** — `generic` if not passed; `why-rf` and venue presets supported by `/slides-polish-codex`.
...
5. **(optional) `— reference: <pdf>`** — visual anchor for `/slides-polish-codex`. If absent and the assurance level requires polish, the workflow uses the Beamer self-compile as reference.
6. **(optional) `— talk_type`, `— minutes`, `— style`, `— effort`** — see Constants.

## Output Layout

File names match `/paper-slides-codex`'s emit contract — this workflow is a
**non-renaming consumer**.

```
slides/
├── SLIDE_OUTLINE.md                # Phase-1 outline (claim-first per slide)
├── main.tex                        # Beamer source (from /paper-slides-codex)
├── main.pdf                        # compiled Beamer
├── presentation.pptx               # editable PPTX (from /paper-slides-codex; emit name fixed by /paper-slides-codex)
├── presentation_pre_polish.pptx    # snapshot before /slides-polish-codex
├── presentation_polished.pptx      # /slides-polish-codex output (when assurance ≥ polished)
├── presentation_polished.pdf       # rendered (LibreOffice or user-export)
├── speaker_notes.md                # per-slide notes (from /paper-slides-codex)
├── TALK_SCRIPT.md                  # full word-for-word talk script + Q&A (from /paper-slides-codex)
...
├── audit-input/                    # Phase-4 staging copies of slide text + notes + script (so /paper-claim-audit-codex and /citation-audit-codex can run on slide content as a synthetic "paper")
...
│   ├── references.bib → ../../../paper/references.bib   # symlink to source paper bib for /citation-audit-codex
...
2. **Check prerequisites**:
   ...
   - PDF tools: `which pdfinfo pdftoppm` (poppler) — required for `/slides-polish-codex` PNG rendering.
...
Otherwise, delegate to `/paper-slides-codex` Phase-1 only (content extraction +
slide outline generation). `/paper-slides-codex` writes the outline into its own
...
Invoke `/paper-slides-codex` to generate Beamer source + PPTX from the approved
outline.

```
/paper-slides-codex "<paper-dir>" — talk_type: <T> — minutes: <N> — venue: <V> — aspect: 16:9 — notes: true
```
...
`/paper-slides-codex` produces (emit names fixed by `/paper-slides-codex`):
...
Invoke `/slides-polish-codex` against the freshly generated PPTX, with a
...
```
/slides-polish-codex "slides/presentation.pptx" — reference: slides/main.pdf — style: <preset> — effort: <effort>
```
...
`/slides-polish-codex` emits a `BLOCKED` verdict because a Codex fix proposal
...
`/paper-claim-audit-codex` and `/citation-audit-codex` expect a paper-shaped input
...
The staged "paper" mirrors `/paper-claim-audit-codex` and `/citation-audit-codex`'s
...
Invoke `/paper-claim-audit-codex` against the staged input. Scope is **slide
...
```
/paper-claim-audit-codex ".aris/paper-talk/audit-input"
```
...
Invoke `/citation-audit-codex` over the staged input. Verify any `\cite{...}` in
slides + notes + script via DBLP / CrossRef; flag fabricated entries.

```
/citation-audit-codex ".aris/paper-talk/audit-input"
```
...
| `effort` | Behavior |
|---|---|
| `lite` | Forwarded to sub-skills. `/slides-polish-codex` runs in `lite` (BLOCKERS only). **Phase 4 still runs at the requested assurance level** — `effort` controls depth, never audit gating (see `shared-references/assurance-contract.md`). |
| `balanced` (default) | Standard pipeline. `/slides-polish-codex` runs `balanced`. |
| `max` | `/slides-polish-codex` runs `max` (per-page review on every slide). Phase-4 audits read all artefacts in full. |
| `beast` | `/slides-polish-codex` runs `beast` (second polish round). Phase-4 audits include extended assurance checks; final report adds an executive summary. |
...
```
/paper-talk-codex "paper/" — talk_type: oral — minutes: 25 — assurance: conference-ready \
            — reference: ./refs/why_rf_2025.pdf — style: why-rf — effort: max
```

Forwarded to:

- `/paper-slides-codex` (Phase 2): `paper-dir`, `talk_type`, `minutes`, `venue`, `aspect`, `notes`, `style-ref` if writer-side.
- `/slides-polish-codex` (Phase 3): polished pptx target, `reference`, `style`, `effort`.
- `/paper-claim-audit-codex` (Phase 4.1): scoped to slide deck artefacts (not paper).
- `/citation-audit-codex` (Phase 4.2): scoped to slide-deck-only citations.
- Anonymity scan (Phase 4.3): inline tool, no external skill.

## When NOT to Use

- The paper is not yet compiled or claims are not stable. Run `/paper-writing-codex` first.
- The user wants a poster, not a talk. Use `/paper-poster-html-codex`.
- The user already has a finished deck and only needs visual polish. Use `/slides-polish-codex` directly — `/paper-talk-codex` would needlessly rebuild.
- The talk content is unrelated to a paper (general lecture, demo). The orchestration assumes a paper-grounded talk; for ad-hoc decks, use `/paper-slides-codex` directly with manual outline.

## Prior Skill Relationship

- Composes `/paper-slides-codex`, `/slides-polish-codex`, `/paper-claim-audit-codex`, `/citation-audit-codex`.
- Does **not** call `/kill-argument-codex` by default — that is upstream intellectual stress-test, not deck QA. Users who want talk-story stress-test before slide build can run `/kill-argument-codex paper/` first.
- Sister workflow to `/paper-writing-codex` (paper) and `/paper-poster-html-codex` (poster).
