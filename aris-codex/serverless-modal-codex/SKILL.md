...
The most common pattern for `run-experiment-codex` integration. Wraps an existing training script:
...
```
/run-experiment-codex "train model"       <- detects gpu: modal, calls /serverless-modal-codex
  -> /serverless-modal-codex              <- analyzes task, generates launcher, runs
  -> Results returned locally or to Modal Volume
  -> No destroy step needed (auto scale-to-zero)

/serverless-modal-codex                   <- standalone: any Modal GPU workload
/serverless-modal-codex "deploy vLLM"     <- inference service deployment
```
