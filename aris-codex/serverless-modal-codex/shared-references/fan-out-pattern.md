...
primitive at all (`/kill-argument-codex` runs two sequential fresh codex
threads with **no Agent tool**; `/citation-audit-codex` verifies per-entry;
`/proof-checker-codex` re-derives per-round). A richer runtime (ultracode /
...
|---|---|
| Idea generation across lenses | `/novelty-check-codex` — the verdict IS the product |
| Literature retrieval across sources | `/research-review-codex` — single heterogeneous critique |
| Attack-angle enumeration | `/experiment-audit-codex` — one cross-model integrity ruling |
...
**judgment** skill across Claude clones. `/novelty-check-codex`,
`/research-review-codex`, `/experiment-audit-codex`, and the `/peer-review`
...
### `/kill-argument-codex` — Tier 3 sequential fan-out, NO Agent tool

`/kill-argument-codex` is the canonical proof that fan-out is a prompt pattern,
...
### `/idea-creator-codex` — Tier-1 parallel lens fan-out → dedup → classified jury

`/idea-creator-codex` fans out idea generation across analytic *lenses*
...
idea and ranks for a top venue. `/idea-creator-codex` declares the `Agent` tool — re-granted (per the re-grant
...
> Today `/idea-creator-codex` Phase 3 (`skills/idea-creator/SKILL.md:159,175`)
...
### `/research-lit-codex` — per-source fan-out, deterministic gate as "jury"

`/research-lit-codex` fans out retrieval across sources (arXiv, Semantic
...
- **Tier-3** (sequential fallback) spawns nothing — e.g. `/kill-argument-codex`
  runs its two passes as fresh Codex reviewer threads, no Agent tool.
  Correctly, `kill-argument-codex` does **not** grant `Agent`.
...
`idea-creator-codex`, `proof-checker-codex`, and `research-lit-codex`. Note that "reviewer
