...
- **Verdict re-run on a wall-clock timer.** Wrapping
  `/auto-review-loop-codex` in `/loop 30m` does not produce 30-minutes-better
...
- **Duplicated scheduling.** `/experiment-queue-codex` already runs a
...
- **GPU / experiment job completion polling.**
  `/monitor-experiment-codex` + `/check-gpu` on a cadence: "is the job done?
...
- **WandB anomaly checks.** `/training-check-codex` is *already* cron-wired:
...
- **Overnight `research-pipeline-codex` heartbeat.** A non-judgmental wake
...
- **Daily literature watch.** A once-a-day `/research-lit-codex` or
  `/deepxiv-codex` sweep for new arXiv papers in a tracked direction. The
...
- **`/loop` around `/auto-review-loop-codex`.** The auto-review loop *is*
...
- **Polling `/experiment-queue-codex` on a timer.** Duplicates the queue's
...
- `/auto-review-loop-codex` — already loops internally; reviewer carries
  round-to-round memory in one `threadId` (`codex-reply`)
- `/auto-review-loop-llm-codex`, `/auto-review-loop-minimax-codex` — same loop, alternate
...
- `/auto-paper-improvement-loop-codex` — review → fix → recompile loop with its own
  round structure and a fresh-reviewer bias guard each round (no `codex-reply`)
- `/research-review-codex` — produces a traced review verdict (provisional in base Codex)
- `/result-to-claim-codex` — judges whether results support a claim
- `/experiment-audit-codex` — judges experiment integrity
- `/paper-claim-audit-codex` — judges paper-to-evidence fidelity
- `/citation-audit-codex` — judges bibliographic correctness
- `/proof-checker-codex` — judges proof validity across rounds
- `/kill-argument-codex` — adversarial accept/reject verdict
...
> **Adjacent but distinct — `/dse-loop-codex`.** It also loops internally, so do
...
> duplication** (component #4 below), the same reason as `/experiment-queue-codex`,
...
- `/monitor-experiment-codex` — poll for job completion / progress
- `/check-gpu` — poll for GPU availability and running processes
- `/experiment-queue-codex` — **visibility only** (report position); never a
  re-poll that competes with its own scheduler
- overnight `/research-pipeline-codex` — a **non-judgmental heartbeat + nudge**
  (see next), never a quality gate

## The overnight-pipeline rule

An overnight `research-pipeline-codex` heartbeat may wake on a cadence,
