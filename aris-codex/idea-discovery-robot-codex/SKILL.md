---
name: idea-discovery-robot-codex
...
```
/research-lit-codex → /idea-creator-codex (robotics framing) → /novelty-check-codex → /research-review-codex
...
- **REVIEWER_MODEL = `gpt-6-astra`** — External reviewer model via a secondary Codex agent
- **TARGET_VENUES = CoRL, RSS, ICRA, IROS, RA-L** — Default novelty and reviewer framing

> Override inline, e.g. `/idea-discovery-robot-codex "bimanual manipulation" — only sim ideas, no real robot` or `/idea-discovery-robot-codex "drone navigation" — focus on CoRL/RSS, 2 pilot ideas max`
...
```
/research-lit-codex "$ARGUMENTS — focus venues: CoRL, RSS, ICRA, IROS, RA-L, TRO, Science Robotics"
```
...
```
/idea-creator-codex "$ARGUMENTS — robotics frame: [paste Robotics Problem Frame] — focus venues: CoRL, RSS, ICRA, IROS, RA-L — benchmark-specific ideas only — sim-first pilots — no real-robot execution without explicit approval — require failure metrics and baseline clarity"
```
...
```
/novelty-check-codex "[idea description with embodiment + task family + benchmark + sensor stack + controller/policy class + sim2real angle + target venues: CoRL/RSS/ICRA/IROS/RA-L]"
```
...
```
/research-review-codex "[top idea with robotics framing, embodiment, benchmark, baselines, pilot plan, evaluation metrics, and sim2real/hardware risks — review as CoRL/RSS/ICRA reviewer]"
```
...
- [ ] Run /novelty-check-codex on the final idea wording
...
```
/idea-discovery-robot-codex "direction"   ← you are here
implement sim-first pilot
/run-experiment-codex                     ← if infrastructure exists
/auto-review-loop-codex "top robotics idea"
```
