...
- **ARIS** can read your changes, run audits (`/paper-claim-audit-codex`, `/citation-audit-codex`, `/auto-paper-improvement-loop-codex`), and push fixes back
...
- **Number changes** that should re-trigger `/paper-claim-audit-codex`
- **Cite key changes** that should re-trigger `/citation-audit-codex`
...
| Numerical / claim change | Sync, then re-run `/paper-claim-audit-codex` |
| New `\cite{...}` | Sync, then re-run `/citation-audit-codex` |
...
- If the user is in an active Overleaf editing session, ARIS skills should **read-only** access `paper/` until the user runs `/overleaf-sync-codex pull`.
- If ARIS is in the middle of `/auto-paper-improvement-loop-codex` or `/paper-write-codex`, the user should pause Overleaf editing until the loop finishes and `/overleaf-sync-codex push` is run.
...
- `/paper-claim-audit-codex` — re-run after pulling Overleaf changes that touch numbers
- `/citation-audit-codex` — re-run after pulling Overleaf changes that add/edit `\cite{...}`
- `/paper-compile-codex` — local LaTeX build; Overleaf compiles independently in the cloud
