...
`/idea-creator-codex`, fetched paper abstracts, and **community-PR-authored
...
- **research-wiki** (`tools/research_wiki.py`): edge `evidence` is quarantined
  ...
  the `query_pack` (injected into `/idea-creator-codex`) is scanned at rebuild time and,
  ...
  scan-and-banner.)
- **`/idea-creator-codex` query-pack load**: both the main skill and Codex mirror scan
...
- **To extend** (same helper, same scopes): MEMORY.md write + load; fetched
  abstracts (`research-lit-codex` / `exa-search-codex` / `deepxiv` / `alphaxiv`) at
