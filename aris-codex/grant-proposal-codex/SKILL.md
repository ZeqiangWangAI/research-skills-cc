...
```
/research-lit-codex → /novelty-check-codex → [structure design] → [draft] → /research-review-codex → [revise] → GRANT_PROPOSAL.md
  (survey)      (verify gap)     (aims + matrix)     (prose)    (panel review)     (fix)      (done!)
```

**This is a parallel branch, not part of the linear Workflow 1→1.5→2→3 pipeline.** After `/idea-discovery-codex` produces validated ideas, the user can either:
- Go to `/experiment-bridge-codex` → `/auto-review-loop-codex` → `/paper-writing-codex` (implement & publish)
- Go to `/grant-proposal-codex` (write funding application first, then implement after funding)

```
                    ┌→ /experiment-bridge-codex → /auto-review-loop-codex → /paper-writing-codex  (publish track)
/idea-discovery-codex ────┤
                    └→ /grant-proposal-codex → [get funded] → /experiment-bridge-codex → ...  (funding track)
```
...
- **GRANT_TYPE = `KAKENHI`** — Default grant type. Supported: `KAKENHI`, `NSF`, `NSFC`, `ERC`, `DFG`, `SNSF`, `ARC`, `NWO`, `GENERIC`. Override via argument (e.g., `/grant-proposal-codex "topic — NSF"`).
...
> 💡 These are defaults. Override by telling the skill, e.g., `/grant-proposal-codex "topic — NSF CAREER, latex output"` or `/grant-proposal-codex "topic — NSFC Youth, language: English"`.
...
```
/grant-proposal-codex "topic — GENERIC, sections: Background|Methods|Impact, language: English"
```
...
1. Read `idea-stage/IDEA_REPORT.md` if it exists (from `/idea-discovery-codex`); fall back to `./IDEA_REPORT.md` if not found
2. Read `refine-logs/FINAL_PROPOSAL.md` if it exists (from `/research-refine-codex`)
3. Read `refine-logs/EXPERIMENT_PLAN.md` if it exists (from `/experiment-plan-codex`)
4. Read `review-stage/AUTO_REVIEW.md` if it exists (from `/auto-review-loop-codex` — prior review feedback is gold for grants); fall back to `./AUTO_REVIEW.md` if not found
...
- No research idea at all → suggest running `/idea-discovery-codex` first
- No literature survey → will invoke `/research-lit-codex` inline in Phase 1
...
Invoke `/research-lit-codex` to ground the proposal in real literature, then search for competing funded projects:

```
/research-lit-codex "$ARGUMENTS"
```

**What this does:**
- Reuse existing surveys if `/research-lit-codex` was already run and notes exist
- Otherwise invoke `/research-lit-codex` for multi-source literature search (arXiv, Scholar, Zotero, local PDFs)
...
- Run `/novelty-check-codex` on the proposed research direction to verify the gap is real:
  ```
  /novelty-check-codex "[proposed gap statement]"
  ```
...
Invoke `/research-review-codex` to get critical feedback on the proposal structure before drafting:

```
/research-review-codex "[GRANT_TYPE] [GRANT_SUBTYPE] proposal structure:
Gap: [gap statement]
...
- Uses `/paper-illustration-codex` for figure generation (if user requests)
...
For AI-generated publication-quality figures, invoke `/paper-illustration-codex`:

```
/paper-illustration-codex "Overview diagram showing [aims relationship + shared resources] for grant proposal"
```
...
Invoke `/research-review-codex` on the complete draft for grant-type-specific evaluation:

```
/research-review-codex "Complete [GRANT_TYPE] [GRANT_SUBTYPE] proposal draft. Evaluate as a [GRANT_TYPE] review panelist using official criteria. [PASTE FULL PROPOSAL TEXT]"
```
...
> ⚠️ **External review fallback**: If reviewer agents are unavailable, skip external review. Note "External review skipped — no reviewer agent available. Consider running `/auto-review-loop-llm-codex` separately." in `GRANT_REVIEW.md`. The proposal is still usable without external review.

If `/research-review-codex` is invoked (preferred), it handles the external review internally. If you run the reviewer directly, use `spawn_agent` for Round 1 and `send_input` for follow-up rounds.
...
```
/grant-proposal-codex "topic — KAKENHI Start-up, sources: zotero, arxiv download: true"
```
...
| `sources` | all | Literature sources | → `/research-lit-codex` |
| `arxiv download` | false | Download arXiv PDFs | → `/research-lit-codex` |
...
|-----------|:-----:|---------|
| `/research-lit-codex` | 1 | Literature survey (if not already done) |
| `/novelty-check-codex` | 1 | Verify the gap is real |
| `/research-review-codex` | 2, 4 | Structural review + full draft review |
| `/paper-illustration-codex` | 3 | Generate proposal figures (optional) |

### Funding Track (this skill's primary use case)

```
/idea-discovery-codex "direction"              ← Workflow 1: find validated ideas
/research-refine-codex "idea"                  ← sharpen the method
/grant-proposal-codex "idea — KAKENHI"         ← this skill: write the grant proposal
                                          ← [submit & get funded]
/experiment-bridge-codex                       ← implement experiments with funding
/auto-review-loop-codex "results"              ← Workflow 2: iterate until submission-ready
/paper-writing-codex                           ← Workflow 3: write the paper
```

### Publish Track (skip this skill)

```
/idea-discovery-codex → /experiment-bridge-codex → /auto-review-loop-codex → /paper-writing-codex → submit
```
