---
name: feishu-notify-codex
description: "Send notifications to Feishu/Lark. Internal utility used by other skills, or manually via /feishu-notify-codex. Use when user says \"发飞书\", \"notify feishu\", or other skills need to send status updates."
---
...
| Skill | Event | When |
|-------|-------|------|
| `/auto-review-loop-codex` | `review_scored` | After each round's review score |
| `/auto-review-loop-codex` | `pipeline_done` | Loop complete (positive or max rounds) |
| `/auto-paper-improvement-loop-codex` | `review_scored` | After each round's review score |
| `/auto-paper-improvement-loop-codex` | `pipeline_done` | All rounds complete |
| `/run-experiment-codex` | `experiment_done` | Screen session finishes |
| `/idea-discovery-codex` | `checkpoint` | Between phases (if interactive) |
| `/idea-discovery-codex` | `pipeline_done` | Final report ready |
| `/monitor-experiment-codex` | `experiment_done` | Results collected |
| `/research-pipeline-codex` | `checkpoint` | Between workflow stages |
| `/research-pipeline-codex` | `pipeline_done` | Full pipeline complete |
