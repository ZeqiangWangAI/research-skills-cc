...
ARIS's claim audits (`/result-to-claim-codex`, `/experiment-audit-codex`, `/paper-claim-audit-codex`)
...
- **`/result-to-claim-codex`** Step 1.5: parse each claim's cited `(value, source)`,
...
- **To extend:** `/experiment-audit-codex` (the "phantom results" check is exactly
  this) and `/paper-claim-audit-codex` (every reported number → its result file).
