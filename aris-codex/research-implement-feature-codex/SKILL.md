---
name: research-implement-feature-codex
...
| "find me a research direction and take it to a paper" | `/research-pipeline-codex` |
| "I have `EXPERIMENT_PLAN.md` — run the campaign" | `/experiment-bridge-codex` |
| "sweep these parameters" | `/dse-loop-codex` |
| "launch what is already written" | `/run-experiment-codex` |
| "do these results support the claim?" | `/result-to-claim-codex` |

`/research-pipeline-codex` decides *what to research*; this skill decides **nothing**
...
| "the implementation is *correct* / the method *works*" | **B** | **out of scope** — `/experiment-audit-codex`, `/result-to-claim-codex` |
...
7. **Next** — this skill again for the next rung, `/run-experiment-codex` to launch, or
   `/experiment-audit-codex` / `/result-to-claim-codex` before anything becomes a claim.
