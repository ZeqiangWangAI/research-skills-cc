...
- **REVIEWER_MODEL = `gpt-6-astra`** — Used via Codex MCP for internal stress-testing.
...
- **AUTO_EXPERIMENT = false** — When `true`, automatically invoke `/experiment-bridge-codex` to run supplementary experiments when the strategy plan identifies reviewer concerns that require new empirical evidence. When `false` (default), pause and present the evidence gap to the user for manual handling.
...
- **RENDER_HTML = true** — When `true` (default), auto-render `rebuttal/REBUTTAL_DRAFT_rich.md` to HTML after Phase 6 / Phase 8 finalization via `/render-html-codex`. Uses **full review gate** (reviewer-facing pre-submission deliverable). The plain-text `PASTE_READY.txt` is NOT rendered (it's character-counted plain text by design). Set `false` to skip, or pass `— render html: false`. **Non-blocking**: failures don't invalidate the rebuttal.

> Override: `/rebuttal-codex "paper/" — venue: NeurIPS, character limit: 5000`
...
**QUICK_MODE exit**: If `QUICK_MODE = true`, stop here. Present `ISSUE_BOARD.md` + `STRATEGY_PLAN.md` to the user and summarize: how many issues per reviewer, shared vs unique concerns, recommended priorities, and evidence gaps. The user can then decide to continue with full rebuttal (`/rebuttal-codex — quick mode: false`) or write manually.
...
2. Invoke `/experiment-bridge-codex` with the mini plan:
   ```
   /experiment-bridge-codex "rebuttal/REBUTTAL_EXPERIMENT_PLAN.md"
   ```
...
```
/render-html-codex "rebuttal/REBUTTAL_DRAFT_rich.md"
```
...
**Non-blocking**: if `/render-html-codex` fails, log the failure and treat the rebuttal phase as complete — the `PASTE_READY.txt` and `REBUTTAL_DRAFT_rich.md` are the canonical outputs.
