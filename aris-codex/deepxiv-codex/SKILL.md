...
- **DEEPXIV_FETCHER** — canonical name `deepxiv_fetch.py`, resolved per
  ...
  (Codex-side chain: `$ARIS_REPO/tools/` → `tools/` → `~/.codex/skills/deepxiv-codex/`).
  Policy D1 — if unresolved (canonical chain exhausted), fall back to raw `deepxiv` CLI.
- **MAX_RESULTS = 10** — Default number of search results.

> Overrides (append to arguments):
> - `/deepxiv-codex "agent memory" - max: 5`
> - `/deepxiv-codex "2409.05591" - brief`
> - `/deepxiv-codex "2409.05591" - head`
> - `/deepxiv-codex "2409.05591" - section: Introduction`
> - `/deepxiv-codex "trending" - days: 14 - max: 10`
> - `/deepxiv-codex "karpathy" - web`
> - `/deepxiv-codex "258001" - sc`
...
- If DeepXiv is missing, give the install command and suggest `/arxiv-codex` or `/research-lit-codex "topic" - sources: web`.
