...
- AUDIT_DOC: `PROOF_AUDIT.md` at the paper directory root, alongside `main.tex` (cumulative log; when invoked via `/paper-writing-codex`, this is `paper/PROOF_AUDIT.md`)
...
- **RENDER_HTML = true** — When `true` (default), auto-render `PROOF_AUDIT.md` to HTML at workflow end via `/render-html-codex`. Uses **full review gate** (audit-class, math-heavy — render-fidelity check protects against MathJax breakage). Set `false` to skip, or pass `— render html: false`. **Non-blocking**: failures don't invalidate the proof audit.
...
separate axis carried by edges (`/result-to-claim-codex`), never written into `status`.
...
| `PROOF_AUDIT.html` (+ `.review.json` sidecar) | Single-file HTML view auto-rendered via `/render-html-codex "PROOF_AUDIT.md" --json "PROOF_AUDIT.json"`. **Non-blocking** — if `/render-html-codex` fails the audit still counts as complete. | Workflow end (when `RENDER_HTML = true`, default) |

## Submission Artifact Emission

This skill **always** writes `PROOF_AUDIT.json` at the paper directory
root (i.e. `paper/PROOF_AUDIT.json` when invoked from `/paper-writing-codex`
...
verdict `NOT_APPLICABLE`; silent skip is forbidden. `paper-writing-codex`
...
```
/proof-checker-codex "neurips_2025.tex"
/proof-checker-codex "check the GMM generalization proof, focus on dimension dependence"
/proof-checker-codex "verify proof in paper.tex — difficulty: nightmare"
```
