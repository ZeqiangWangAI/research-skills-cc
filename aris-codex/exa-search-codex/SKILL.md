...
| Skill | Best for |
|------|----------|
| `/arxiv-codex` | Direct preprint search and PDF download |
| `/semantic-scholar-codex` | Published venue papers (IEEE, ACM, Springer), citation counts |
| `/deepxiv-codex` | Layered reading: search, brief, section map, section reads |
| `/exa-search-codex` | Broad web search: blogs, docs, news, companies, research papers — with content extraction |
...
- **EXA_FETCHER** — canonical name `exa_search.py`, resolved per
  ...
  (Codex-side chain: `$ARIS_REPO/tools/` → `tools/` → `~/.codex/skills/exa-search-codex/`).
  Policy D1 — standalone `/exa-search-codex` has no documented fallback,
  so unresolved helper terminates with an explicit error.
- **MAX_RESULTS = 10** — Default number of results to return.

> Overrides (append to arguments):
> - `/exa-search-codex "RAG pipelines" — max: 5` — top 5 results
> - `/exa-search-codex "diffusion models" — category: research paper` — research papers only
> - `/exa-search-codex "startup funding" — category: news, start date: 2025-01-01` — recent news
> - `/exa-search-codex "transformer" — content: text, max chars: 8000` — full text mode
> - `/exa-search-codex "transformer" — content: summary` — LLM-generated summaries
> - `/exa-search-codex "transformer" — domains: arxiv.org,huggingface.co` — domain filter
> - `/exa-search-codex "https://arxiv.org/abs/2301.07041" — similar` — find similar pages
...
[ -z "$EXA_FETCHER" ] && {
  echo "ERROR: exa_search.py not resolved at \$ARIS_REPO/tools/, tools/, or ~/.codex/skills/exa-search-codex/." >&2
  echo "       Fix: rerun tools/install_aris_codex.sh, export ARIS_REPO, or copy the helper to ~/.codex/skills/exa-search-codex/." >&2
...
- Combine with `/arxiv-codex` or `/semantic-scholar-codex` for comprehensive literature coverage
