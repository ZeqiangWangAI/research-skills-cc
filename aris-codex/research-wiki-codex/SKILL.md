...
| **Claim** | `claims/` | `claim:<id>` | A theorem/headline with an honest PROOF status — born via `/proof-checker-codex` (see Hook 4) |
...
research-wiki/
  ...
  query_pack.md          # compressed summary for /idea-creator-codex (auto-generated, max 8000 chars)
  papers/
...
### `/research-wiki-codex ingest "<paper title>" — arxiv: <id>`
...
Other skills (`/research-lit-codex`, `/arxiv-codex`, `/alphaxiv-codex`, `/deepxiv-codex`,
`/semantic-scholar-codex`, `/exa-search-codex`) call the same helper directly in
their own last step — they don't re-route through `/research-wiki-codex
ingest` as a subcommand, so they don't need an LLM roundtrip.

### `/research-wiki-codex sync — arxiv-ids <id1>,<id2>,...`
...
### `/research-wiki-codex query "<topic>"`
...
### `/research-wiki-codex update <node_id> — <field>: <value>`

Update a specific entity:

```
/research-wiki-codex update paper:chen2025 — relevance: core
/research-wiki-codex update idea:001 — outcome: negative
/research-wiki-codex update claim:C1 — status: refuted
```

After any update: rebuild `query_pack.md`, update `log.md`.

### `/research-wiki-codex lint`
...
### Hook 1: After `/research-lit-codex` finds papers
...
### Hook 2: `/idea-creator-codex` reads AND writes wiki
...
for idea in all_generated_ideas (recommended + killed):
    ...
    # by /result-to-claim-codex). `outcome` ∈ {unknown,pending,negative,mixed,positive} — the
    # experiment verdict is set later by /result-to-claim-codex, never guessed at ideation.
log "idea-creator wrote N ideas to wiki"
```

### Hook 3: After `/result-to-claim-codex` verdict
...
# refuted / unproven / drafted / retracted), owned by /proof-checker-codex (Hook 4). The ARIS
...
### Hook 4: Claim birth — from `/proof-checker-codex` (the ONLY birth point)

Wiki **claim nodes are born here.** `/proof-checker-codex` mints a claim node for each
...
`/result-to-claim-codex` (Hook 3) only adds empirical `supports`/`invalidates` edges to an
...
After significant wiki updates, suggest re-running `/idea-creator-codex`:
