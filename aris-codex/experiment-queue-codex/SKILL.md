...
Use when `/run-experiment-codex` is insufficient:
...
Do NOT use for:
- Single ad-hoc experiment (use `/run-experiment-codex`)
...
Note: `/monitor-experiment-codex` is currently focused on screen sessions, result JSONs, and W&B; it does not yet read `queue_state.json` directly. For queue-state monitoring, use the literal command above.
...
- Local skill agent invokes `/analyze-results-codex` if `analyze_on_complete: true`.
...
## Next Steps
- Run `/analyze-results-codex` on output JSONs
- Figures auto-regen via `artifact-sync` (if configured)
```

## Comparison with `/run-experiment-codex`

| Feature | `/run-experiment-codex` | `experiment-queue-codex` |
| --- | --- | --- |
...
**Rule**: Use `/run-experiment-codex` for ≤5 jobs. Use `experiment-queue-codex` for ≥10 jobs or anything with phases.
...
Claude invokes `/experiment-queue-codex`:
...
- `/run-experiment-codex` — single experiment deployment
- `/monitor-experiment-codex` — check progress (now reads from queue_state.json)
- `/analyze-results-codex` — post-hoc analysis
